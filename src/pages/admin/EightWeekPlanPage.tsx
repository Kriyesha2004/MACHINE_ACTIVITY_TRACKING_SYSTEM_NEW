...
export const EightWeekPlanPage: React.FC = () => {
    ...
    const fetchPlans = async () => {
        try {
            const response = await fetch('http://localhost:3000/maintenance/plans/active?frequency=8-Weekly');
    ...
    return (
        <div className="w-full p-6">
             <PageHeader 
                title="8-Week Maintenance Plans" 
                subtitle="Manage assignments and view history for 8-week maintenance cycles" 
                action={
                    <Link to="/admin/8-week/planner" className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-500 transition-colors">
                        <Calendar size={16} />
                        Plan New Cycle
                    </Link>
                }
            ...
            <div className="space-y-4">
                {loading ? (
                    ...
                    <div className="text-center text-slate-400 py-12">No 8-week plans found. Use the Planner to create new cycles.</div>
                ) : (
